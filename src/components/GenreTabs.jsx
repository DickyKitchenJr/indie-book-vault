...
function GenreTabs() {
  const tabs = [{src: Children, alt: "Children's", link: "childrens"}, {src: Comedy, alt: "Comedy", link:"comedy"}, {src: Drama, alt: "Drama", link:"drama"}, {src: Dystopian, alt: "Dystopian", link:"dystopian"}, {src: Fantasy, alt: "Fantasy", link:"fantasy"}, {src: Horror, alt: "Horror", link:"horror"}, {src: NonFiction, alt: "Non-Fiction", link:"nonfiction"}, {src: Romance, alt:"Romance", link:"romance"}, {src: SciFi, alt: "Sci-Fi", link:"scifi"}, {src: Thriller, alt: "Thriller", link:"thriller"}, {src: Western, alt: "Western", link:"western"}];
  
  return (
    <>
    <div className={styles.tabs}>
      {tabs.map(tab =>{
        return <Link to={`/${tab.link}`} key={tab.alt}><img src={tab.src} alt={`${tab.alt} genre tab`} className={styles.img}/></Link>
      })}  
    </div>
    </>
  )
